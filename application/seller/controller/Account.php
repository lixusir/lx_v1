...
class Account extends SellerBase{
    ...
    public function rechargeList()
    {
        ...
        if (request()->isPost()) {
            ...
            $where['user_type'] = 2;//用户类型 1业务员 2商家 3管理员
    ...
    public function rechargeEdit(){
        ...
        if(request()->isPost()){
            ...
            if($result){
                $this->success('操作成功',url('Account/rechargeList'));
            }
            $this->error('操作失败');
        }else{
    ...
    public function moneyList()
    {
        ...
        if (request()->isPost()) {
            ...
            $list = accountLogModel::where($where)->order('id desc')->paginate($pageSize)->toArray();
            return $result = ['code' => 0, 'msg' => '获取成功!', 'data' => $list['data'], 'count' => $list['total']];
        }
        $this->assign('linkage',$linkage);
        return view();
